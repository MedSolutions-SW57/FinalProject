...
| Epic / Story ID |                       Título                        |                                                                                                                               Descripción                                                                                                                                |                                                                                                                                                                                                                                                                                                                                                                                                                                  Criterios de Aceptación                                                                                                                                                                                                                                                                                                                                                                                                                                    | Relacionado con (Epic ID) |
| :-------------: | :-------------------------------------------------: | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------: |:-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------:| :-----------------------: |
|      US01       |        Consulta de las citas de los medicos         |                                                                        Como desarrollador quiero obtener las citas programadas para cada doctor para que se puedan mostrar y modificar las notas                                                                         |                                                                   <p>**Escenario 1: Obtener las citas por el id del doctor**</p><p>- Given que el endpoint "doctor/:id/appointments" esta disponible</p><p>- When se envia una solicitud GET con el id del doctor</p><p>- Then se recibe una respuesta con el estado 200<p><p>- And una lista con las informacion de cada appointments de ese doctor estara incluida en el cuerpo de la respuesta</p><p>**Escenario 2: Obtener citas con id de doctor no valido**</p><p>- Given que el endpoint "doctor/:id/appointments" esta disponible</p><p>- When se envia una solicitud GET con el id del doctor</p><p>- Then se recibe una respuesta con el estado 404<p><p>- And se muestra un error en el cuerpo de la respuesta indicando que el doctor no esta registrado</p>                                                                    |            E01            |
|      US02       |      Consulta de tratamientos de los pacientes      |                                                                                                Como desarrollador, obtener los tratatiemientos asignados a cada paciente.                                                                                                |                                                      p>**Escenario 1: Obtener los tratamientos por el id del paciente**</p><p>- Given que el endpoint "patient/:id/treatments" esta disponible</p><p>- When se envia una solicitud GET con el id del paciente</p><p>- Then se recibe una respuesta con el estado 200<p><p>- And en el cuerpo de la respuesta se muestra un lista con los tratamientos asignados a ese paciente</p><p>**Escenario 2: Obtener los tratamientos con id de paciente no valido**</p><p>- Given que el endpoint "patient/:id/treatments" esta disponible</p><p>- When se envia una solicitud GET con el id del paciente</p><p>- Then se recibe una respuesta con el estado 404<p><p>- And se muestra un error en el cuerpo de la respuesta indicando que el paciente no esta registrado en la plataforma</p>                                                      |            E01            |
...
|      US04       |           Gestión de Permisos y Seguridad           | Como desarrollador, quiero implementar un sistema de gestión de permisos y seguridad en MedSystem con roles para asegurar que solo el personal tenga acceso a la información de los pacientes y del sistema, protegiendo así la privacidad y la integridad de los datos. |                                                                                                                                                                                                                                              <p>**Escenario 1: Acceso Seguro y Controlado**<p>- Dado que un empleado de salud intenta acceder a la plataforma MedSystem, </p><p>- Cuando intenta ingresar a la sección de historias clínicas electrónicas,</p><p>- Entonces el sistema debe verificar su rol de DOCTOR o ADMIN, y solo permitir acceso si está autorizado, mostrando un mensaje de error o redireccionando si no lo está.</p>                                                                                                                                                                                                                                               |            E01            |
...
|      US06       |            Consulta de exámenes médicos             |                                                                Como desarrollador, quiero obtener los exámenes médicos a través de la API, para poder mostrar o procesar la información de los exámenes.                                                                 | <p>**Escenario1:Obtener el exámen médico de una paciente con id específico**</p>-Dado que el endpoint "/api/v1/request-result/patient/{patientId}" está disponible,<p> </p>-Cuando se envía una solicitud GET con un patientId válido,<p> </p>-Luego se recibe una respuesta con el estado 200,<p><p>-Y una lista de exámenes médicos para ese paciente está incluida en el cuerpo de la respuesta.</p> </p><p>**Escenario2:Obtener el exámen médico de una paciente con id específico no existente**</p><p>-Dado que el endpoint "/api/v1/request-result/patient/{patientId}" está disponible,</p><p>-Cuando se envía una solicitud GET con un patientId,</p><p>-Y no se encuentra ningún paciente con aquel Id<p>-Luego se recibe una respuesta con el estado 404</p><p>-Luego un mensaje de error está incluido en el cuerpo de la respuesta indicando "Paciente no encontrado".</p></p> |            E01            |
...
| Prepared by                        | Gutierrez Zumaeta, Manuel Alonso                                                                                                                                                                                                                                                                                                                                                                   |
| Attendees (to planning meeting)    | <p>Guimaraes Escalante, Carlos Eduardo</p><p>Altamirano Saenz, Jorge Armando</p><p>Montes Figueroa, Juan Eduardo</p><p>Flores Manrique, Sebastian Enrique</p><p>Gutierrez Zumaeta, Manuel Alonso</p><p></p>                                                                                                                                                                                        |
| Sprint n - 4 Review Summary        | Durante el cuarto sprint, el equipo logró terminar la implementacion del BackEnd y desplegarlo satisfactoriamente, además se hizo los cambios finales para el FrontEnd y LandingPage. Todas las tareas planificadas fueron completadas según las especificaciones requeridas.El despliegue se llevó a cabo sin problemas y el backend está funcionando correctamente.                              |
...
| **Sprint Goal & User Stories**     |                                                                                                                                                                                                                                                                                                                                                                                                    |
| Sprint 4 Goal                      | El objetivo principal de este sprint fue realizar la ultima version del backend y del frontend, en el que se anadieron un nuevo bounded context de laboratorio y la autentificacion                                                                                                                                                                                                                |
